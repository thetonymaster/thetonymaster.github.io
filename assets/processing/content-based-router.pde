...
void mouseReleased(){
  if(clicked){
    ...
    clrs[1] = random(255);
    clrs[2] = random(255);
    messages.add(new Message(125,200, p[0], false, clrs));
  }
}
...
class Message {
  ...
  Message(float xpos1, float ypos1, Pipe p, boolean flag, float[] colorstemp){
    xpos = xpos1;
    ypos = ypos1;
    pipe = p;
  ...
  void move() {
    ...
    if (msgDist > (pipeDist + 10)) {
      ...
      pipe = getPipe();
      if(!finishedFlag){
        messages.add(new Message(250,200, p[pipe+1], true,  colors));
      }
