...
- It can be difficult to do procedures in sequence.
...
2. Get the fibonacci number for an input
...
First, we declare a type, which will have all the methods to expose to the remote procedures.
...
Now, RPC requests can be sent as distributed messaging:
...
Introduction to Distributed Messaging with Elixir - Reactive.TIPS - The Official Blog of Reactive.IO. (n.d.). Retrieved October 25, 2015, from http://www.reactive.io/tips/2015/02/07/introduction-to-distributed-messaging-with-elixir/
